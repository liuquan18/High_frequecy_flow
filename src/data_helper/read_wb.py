...
def convert_geometry(geom):
    ...
    if isinstance(geom, Polygon):
        ...
        new_exterior = [
            (lon if lon <= 180 else lon - 360, lat) for lon, lat in exterior_coords
        ]
        ...
        for interior in geom.interiors:
            ...
            new_interior = [
                (lon if lon <= 180 else lon - 360, lat) for lon, lat in interior_coords
            ]
...
def filter_by_overlap_threshold(gdf, region_box, threshold=0.5):
    ...
    if valid_geometries.shape[0] < gdf.shape[0]:
        logging.warning(
            f"Removed {gdf.shape[0] - valid_geometries.shape[0]} invalid geometries"
        )
