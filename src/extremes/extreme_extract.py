...
def add_pos_sign_times(df:pd.DataFrame, events:pd.DataFrame) -> pd.DataFrame:
    ...
    sign_grouper = df.groupby(df.time.dt.year)['pc'].transform(lambda x: x.lt(0).cumsum())
    ...
    def find_sign_times (row):
        mask = (sign_times['sign_start_time'] <= row['start_time']) & (sign_times['sign_end_time'] >= row['end_time'])
        if mask.any():
    ...
    events['sign_duration'] = (events['sign_end_time'] - events['sign_start_time']).dt.days + 1
