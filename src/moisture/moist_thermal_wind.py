...
def calculate_dry_entropy(T, P):
    """
    Calculate dry entropy (s_d) from temperature and pressure.
    
    Args:
        T (float/array): Temperature [K]
        P (float/array): Pressure [Pa]
    
    Returns:
        s_d (float/array): Dry entropy [J/(kg·K)]
    """
    # Constants
    R_d = 287.0    # Gas constant for dry air [J/(kg·K)]
    c_p = 1005.0   # Specific heat at constant pressure [J/(kg·K)]
    P0 = 100000.0  # Reference pressure [Pa] (typically 1000 hPa = 100000 Pa)

    # Calculate potential temperature (θ)
    theta = T * (P0 / P) ** (R_d / c_p)
    
    # Calculate dry entropy
    s_d = c_p * np.log(theta)
...
def dry_entropy(arr):
    ...
    Args:
        arr (xarray.DataArray): Temperature [K] and pressure [Pa]
    
    Returns:
        s_d (xarray.DataArray): Dry entropy [J/(kg·K)]
    """
    return xr.apply_ufunc(
        calculate_dry_entropy,
        arr,
        arr.plev,
        input_core_dims=[["plev"], ["plev"]],
        output_core_dims=[["plev"]],
