...
#%%
daily_field_path = f"/work/mh0033/m300883/High_frequecy_flow/data/MPI_GE_CMIP6/daily/zg_MJJAS_ano_{period}"
eof_path = "/work/mh0033/m300883/High_frequecy_flow/data/MPI_GE_CMIP6/season/EOF_result/troposphere_ind_decade_first_JJA_eof_result.nc"
...
for i, member in enumerate(members_single):
    ...
    eof = xr.open_dataset(eof_path).eof.sel(mode = 'NAO', decade = year_range[:4]).squeeze()

    projected_pcs = project_field_to_pattern(daily_field, eof, standard=False)

    projected_pcs.name = "pc"
    projected_pcs.to_netcdf(
        f"/work/mh0033/m300883/High_frequecy_flow/data/MPI_GE_CMIP6/daily/projected_pc_{period}_nonstd/troposphere_pc_MJJAS_ano_{year_range}_r{member}_nonstd.nc"
    )
