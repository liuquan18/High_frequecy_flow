...
def event_pc(period, duration):
    ...
    for i in range(50):
        pc = xr.open_dataset(f'{pc_path}projected_pc_{period}/zg_JJA_ano_{tag}_r{i+1}.nc').pc
    ...
    pos_pcs = pd.concat(pos_pcs, axis=1) if pos_pcs else pd.DataFrame().dropna(axis = 0, how = 'all')
    neg_pcs = pd.concat(neg_pcs, axis=1) if neg_pcs else pd.DataFrame().dropna(axis = 0, how = 'all')
...
duration = 5
for ax in axes.flat:
    _, _, first10_pos_pcs, first10_neg_pcs = event_pc('first10', duration)
    _, _, last10_pos_pcs, last10_neg_pcs = event_pc('last10', duration)
