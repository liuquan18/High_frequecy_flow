...
hus_va_Cxy_NPC = read_Cxy("hus", "va", "NPC", pixel_wise=True)
# %%
tas_va_Cxy_NAL = read_Cxy("tas", "va", "NAL", pixel_wise=True)
tas_va_Cxy_NPC = read_Cxy("tas", "va", "NPC", pixel_wise=True)
...
hus_va_NPC_mean = hus_va_Cxy_NPC.mean(dim=("time", "lat", "lon"))

tas_va_NAL_mean = tas_va_Cxy_NAL.mean(dim=("time", "lat", "lon"))
tas_va_NPC_mean = tas_va_Cxy_NPC.mean(dim=("time", "lat", "lon"))
...
hus_va_NPC_mean_smooth = smooth_period(hus_va_NPC_mean)

tas_va_NAL_mean_smooth = smooth_period(tas_va_NAL_mean)
tas_va_NPC_mean_smooth = smooth_period(tas_va_NPC_mean)
...
# smooth
axes[0].plot(
    1 / f, hus_va_NPC_mean_smooth, color=prec_cmap_seq(0.9), linewidth=2, linestyle="--"
)
axes[0].plot(
    1 / f, tas_va_NPC_mean_smooth, color=temp_cmap_seq(0.7), linewidth=2, linestyle="--"
)

axes[1].plot(
    1 / f, hus_va_NAL_mean_smooth, color=prec_cmap_seq(0.9), linewidth=2, linestyle="--"
)
axes[1].plot(
    1 / f, tas_va_NAL_mean_smooth, color=temp_cmap_seq(0.7), linewidth=2, linestyle="--"
)

axes[0].set_title(r"$\Delta q_{NPC}$,  $va_{NPC}$ ")
...
plt.tight_layout()
plt.savefig(
    "/work/mh0033/m300883/High_frequecy_flow/docs/plots/mositure_paper_v1/coherence_ERA5.png",
    dpi=300,
)
