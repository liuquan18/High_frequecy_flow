...
def event_composite(period, jet_plev=None):
    ...
    for ens in range(1, 51):
        ...
        if not NAO_pos.empty:
            ...
            NAO_pos_AWB = composite_before_NAO(
                NAO_pos, AWB, lag_days=[-5, 5], WB_type="AWB"
            )
            NAO_pos_CWB = composite_before_NAO(
                NAO_pos, CWB, lag_days=[-5, 5], WB_type="CWB"
            )
        ...
        if not NAO_neg.empty:
            ...
            NAO_neg_AWB = composite_before_NAO(
                NAO_neg, AWB, lag_days=[-5, 5], WB_type="AWB"
            )
            NAO_neg_CWB = composite_before_NAO(
                NAO_neg, CWB, lag_days=[-5, 5], WB_type="CWB"
            )
...
# %%
# read climatology
first_jet_loc_clim = xr.open_dataset(
...
last_jet_loc_clim = xr.open_dataset(
    "/work/mh0033/m300883/High_frequecy_flow/data/MPI_GE_CMIP6/NA_jet_stream/jet_stream_climatology/jet_loc_climatology_last10.nc"
)
...
kde_plot_during_NAO = sns.kdeplot(
    data=NAO,
    x="jet_loc_during",
    hue="period",
    common_norm=True,
...
axes[2].axvline(
    first_jet_loc_clim.values, color="C0", linestyle="--", label="first10 climatology"
)
axes[2].axvline(
    last_jet_loc_clim.values, color="C1", linestyle="--", label="last10 climatology"
)
...
plt.tight_layout()
plt.savefig(
    "/work/mh0033/m300883/High_frequecy_flow/docs/plots/background_jet/WB_jet_background.png",
    dpi=300,
)
...
axes[2].set_xlabel("Jet location averaged on [-10, -1] days relative to NAO onset")
...
plt.savefig(
    "/work/mh0033/m300883/High_frequecy_flow/docs/plots/background_jet/WB_precursor_jet_background.png",
    dpi=300,
)
...
AWB_jet_mean = NAO[NAO["precusor"] == NAO["AWB"]][['jet_loc_before', 'period']].groupby('period').mean()
CWB_jet_mean = NAO[NAO["precusor"] == NAO["CWB"]][['jet_loc_before', 'period']].groupby('period').mean()

# Add vertical lines at mean of period
axes[0].axvline(
    AWB_jet_mean.loc["first10"].values, color="C0", linestyle="--", label="first10 mean"
)
axes[0].axvline(
    AWB_jet_mean.loc["last10"].values, color="C1", linestyle="--", label="last10 mean"
)

axes[1].axvline(
    CWB_jet_mean.loc["first10"].values, color="C0", linestyle="--", label="first10 mean"
)
axes[1].axvline(
    CWB_jet_mean.loc["last10"].values, color="C1", linestyle="--", label="last10 mean"
)


axes[0].set_title("jet_loc_before when precusor is AWB")
axes[1].set_title("jet_loc_before when precusor is CWB")

plt.tight_layout()
plt.savefig(
    "/work/mh0033/m300883/High_frequecy_flow/docs/plots/background_jet/WB_precursor_jet_background_separate.png",
    dpi=300,
)
# %%
