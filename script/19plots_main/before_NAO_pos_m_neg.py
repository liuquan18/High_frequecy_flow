...
def remove_zonalmean(arr):
    ...
    return arr
#%%
def postprocess(ds, do_smooth = True, remove_zonal = True):
    if do_smooth:
        ds = smooth(ds)
    if remove_zonal:
...
# first column for upvp
upvp_NAO_first.plot.contourf(ax=axes[0, 0], transform=ccrs.PlateCarree(), cmap=temp_cmap_div, levels=upvp_levels_div, extend='both', add_colorbar=False)
upvp_NAO_last.plot.contourf(ax=axes[1, 0], transform=ccrs.PlateCarree(), cmap=temp_cmap_div, levels=upvp_levels_div, extend='both', add_colorbar=False)
upvp_diff = (upvp_NAO_last - upvp_NAO_first).plot.contourf(ax=axes[2, 0], transform=ccrs.PlateCarree(), cmap=temp_cmap_div, levels=upvp_levels_div, extend='both', add_colorbar=False)
cbar_upvp = fig.colorbar(upvp_diff, ax=axes[:, 0], orientation='horizontal', shrink=0.6, pad=0.1)
cbar_upvp.set_label(r'$m^2/s^2$')

# second column for eke
eke_NAO_first.plot.contourf(ax=axes[0, 1], transform=ccrs.PlateCarree(), cmap='RdBu_r', levels=eke_levels_div, extend='both', add_colorbar=False)
eke_NAO_last.plot.contourf(ax=axes[1, 1], transform=ccrs.PlateCarree(), cmap='RdBu_r', levels=eke_levels_div, extend='both', add_colorbar=False)
eke_diff = (eke_NAO_last - eke_NAO_first).plot.contourf(ax=axes[2, 1], transform=ccrs.PlateCarree(), cmap='RdBu_r', levels=eke_levels_div, extend='both', add_colorbar=False)
cbar_eke = fig.colorbar(eke_diff, ax=axes[:, 1], orientation='horizontal', shrink=0.6, pad=0.1)
...
axes[0, 0].set_title(r"1850 $u'v'$")
axes[0, 1].set_title(r"1850 EKE")
axes[1, 0].set_title(r"2090 $u'v'$")
axes[1, 1].set_title(r"2090 EKE")
axes[2, 0].set_title(r"$\Delta u'v'$")
axes[2, 1].set_title(r"$\Delta$ EKE")
