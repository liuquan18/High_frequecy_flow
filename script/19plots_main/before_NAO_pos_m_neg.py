...
fig, axes = plt.subplots(
    3, 3, figsize=(10, 8), subplot_kw={"projection": ccrs.PlateCarree(-90)}
)
...
cbar_uhat = fig.colorbar(uhat_diff, ax=axes[:, 0], orientation="horizontal", pad=1.1)
...
cbar_upvp = fig.colorbar(upvp_diff, ax=axes[:, 1], orientation="horizontal", pad=1.1)
cbar_upvp.set_label(r"$m^2/s^2$")

# third column for eke
ivke_NAO_first.plot.contourf(
    ax=axes[0, 2],
    transform=ccrs.PlateCarree(),
    cmap="RdBu_r",
...
eke_diff = (ivke_NAO_last - ivke_NAO_first).plot.contourf(
    ax=axes[2, 2],
    transform=ccrs.PlateCarree(),
    cmap="RdBu_r",
    levels=eke_levels_div,
...
cbar_eke = fig.colorbar(eke_diff, ax=axes[:, 2], orientation="horizontal", pad=1.1)
