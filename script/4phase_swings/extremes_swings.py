...
def count_swints(swings, percentage = True):
    swings_stat = pd.DataFrame(columns= np.arange(-90, 91, 1),
    ...
    swings_stat.loc['pos_',:] = swings_stat.loc['pos_pos',:] + swings_stat.loc['pos_neg',:]
    swings_stat.loc['neg_',:] = swings_stat.loc['neg_pos',:] + swings_stat.loc['neg_neg',:]

    if percentage:
        swings_stat.iloc[1:3] = swings_stat.iloc[1:3].div(swings_stat.loc['pos_',:], axis=1) * 100
        swings_stat.iloc[4:6] = swings_stat.iloc[4:6].div(swings_stat.loc['neg_',:], axis=1) * 100
...
def plot_swings(swings, swings_perc):
    ...
    for i in [2, 5]:
        ...
        for j, lag in enumerate(lag_days):
            value = swings.loc[row, lag]
            if lag in range(-14, 70, 1):
                ax.bar(lag, 0.4, bottom=i+0.3, width=1., color='white', edgecolor='black')
                ax.text(lag, i+0.5, str(int(value)), ha='center', va='center', fontsize=8)
