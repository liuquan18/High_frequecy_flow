...
def merge_event_ratio(event, ratio, lag=(-20, 10)):
    ...
    expected_ratio_df = pd.DataFrame(
        np.nan, index=event.index, columns=np.arange(lag[0], lag[1] + 1)
    )
...
def sel_before_wb(wb, data, lag=(-20, 10)):
    ...
    for i, event in wb.iterrows():
        ...
        data_wb_event = data.sel(
            time=slice(event_date_20before, event_date_10after), ens=event_ens
        )
        ...
        NAL_ratio, NPC_ratio = ocean_sector(ratio)

        NAL_df = merge_event_ratio(event, NAL_ratio)
        NPC_df = merge_event_ratio(event, NPC_ratio)

        NAL_ratios.append(NAL_df)
        NPC_ratios.append(NPC_df)

    NAL_ratios = pd.concat(NAL_ratios)
    NPC_ratios = pd.concat(NPC_ratios)

    return NAL_ratios, NPC_ratios
...
def process_data(decade):
    ...
    awb_NAL, awb_NPC = sel_before_wb(awb_dec, data)
    cwb_NAL, cwb_NPC = sel_before_wb(cwb_dec, data)

    # save the data
    awb_NAL.to_csv(
        f"/work/mh0033/m300883/High_frequecy_flow/data/MPI_GE_CMIP6/hus_tas_ratio_awb_NAL/awb_NAL_{decade}.csv"
    )
    awb_NPC.to_csv(
        f"/work/mh0033/m300883/High_frequecy_flow/data/MPI_GE_CMIP6/hus_tas_ratio_awb_NPC/awb_NPC_{decade}.csv"
    )

    cwb_NAL.to_csv(
        f"/work/mh0033/m300883/High_frequecy_flow/data/MPI_GE_CMIP6/hus_tas_ratio_cwb_NAL/cwb_NAL_{decade}.csv"
    )
    cwb_NPC.to_csv(
        f"/work/mh0033/m300883/High_frequecy_flow/data/MPI_GE_CMIP6/hus_tas_ratio_cwb_NPC/cwb_NPC_{decade}.csv"
    )
