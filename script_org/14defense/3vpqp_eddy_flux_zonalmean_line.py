...
transient_dfs = transient_dfs[
    (transient_dfs["time"].between(-20, 20)) & (transient_dfs["phase"] != "clima")
]
steady_dfs = steady_dfs[
    (steady_dfs["time"].between(-20, 20)) & (steady_dfs["phase"] != "clima")
]
# %%
...
# second col: transient
transient_first_ano = transient_dfs[
    (transient_dfs["plev"] == 25000) & (transient_dfs["decade"] == 1850)
]
# remove the clima data
transient_first_ano.loc[:, "N"] = (
    transient_first_ano["N"]
    - transient_dfs_clima[
        (transient_dfs_clima["plev"] == 25000) & (transient_dfs_clima["decade"] == 1850)
    ]["N"].values
)
...
transient_last_ano = transient_dfs[
    (transient_dfs["plev"] == 25000) & (transient_dfs["decade"] == 2090)
]
# remove the clima data
transient_last_ano.loc[:, "N"] = (
    transient_last_ano["N"]
    - transient_dfs_clima[
        (transient_dfs_clima["plev"] == 25000) & (transient_dfs_clima["decade"] == 2090)
    ]["N"].values
)
...
# third col: steady
steady_first_ano = steady_dfs[
    (steady_dfs["plev"] == 25000) & (steady_dfs["decade"] == 1850)
]
# remove the clima data
steady_first_ano.loc[:, "N"] = (
    steady_first_ano["N"]
    - steady_dfs_clima[
        (steady_dfs_clima["plev"] == 25000) & (steady_dfs_clima["decade"] == 1850)
    ]["N"].values
...
steady_last_ano = steady_dfs[
    (steady_dfs["plev"] == 25000) & (steady_dfs["decade"] == 2090)
]
# remove the clima data
steady_last_ano.loc[:, "N"] = (
    steady_last_ano["N"]
    - steady_dfs_clima[
        (steady_dfs_clima["plev"] == 25000) & (steady_dfs_clima["decade"] == 2090)
    ]["N"].values
...
# )
# %%
# New: plot only the last row (three line panels) as a separate 1x3 figure
plev_plot = 25000

# prepare sum anomalies for 1850 and 2090
sum_first_ano = sum_dfs[
    (sum_dfs["plev"] == plev_plot) & (sum_dfs["decade"] == 1850)
].copy()
sum_first_ano["N"] = (
    sum_first_ano["N"]
    - sum_dfs_clima[
        (sum_dfs_clima["plev"] == plev_plot) & (sum_dfs_clima["decade"] == 1850)
    ]["N"].values
)

sum_last_ano = sum_dfs[
    (sum_dfs["plev"] == plev_plot) & (sum_dfs["decade"] == 2090)
].copy()
sum_last_ano["N"] = (
    sum_last_ano["N"]
    - sum_dfs_clima[
        (sum_dfs_clima["plev"] == plev_plot) & (sum_dfs_clima["decade"] == 2090)
    ]["N"].values
)

# prepare transient anomalies
transient_first_ano = transient_dfs[
    (transient_dfs["plev"] == plev_plot) & (transient_dfs["decade"] == 1850)
].copy()
transient_first_ano["N"] = (
    transient_first_ano["N"]
    - transient_dfs_clima[
        (transient_dfs_clima["plev"] == plev_plot)
        & (transient_dfs_clima["decade"] == 1850)
    ]["N"].values
)

transient_last_ano = transient_dfs[
    (transient_dfs["plev"] == plev_plot) & (transient_dfs["decade"] == 2090)
].copy()
transient_last_ano["N"] = (
    transient_last_ano["N"]
    - transient_dfs_clima[
        (transient_dfs_clima["plev"] == plev_plot)
        & (transient_dfs_clima["decade"] == 2090)
    ]["N"].values
)

# prepare steady anomalies
steady_first_ano = steady_dfs[
    (steady_dfs["plev"] == plev_plot) & (steady_dfs["decade"] == 1850)
].copy()
steady_first_ano["N"] = (
    steady_first_ano["N"]
    - steady_dfs_clima[
        (steady_dfs_clima["plev"] == plev_plot) & (steady_dfs_clima["decade"] == 1850)
    ]["N"].values
)

steady_last_ano = steady_dfs[
    (steady_dfs["plev"] == plev_plot) & (steady_dfs["decade"] == 2090)
].copy()
steady_last_ano["N"] = (
    steady_last_ano["N"]
    - steady_dfs_clima[
        (steady_dfs_clima["plev"] == plev_plot) & (steady_dfs_clima["decade"] == 2090)
    ]["N"].values
)
...
fig_lastrow, axs = plt.subplots(1, 3, figsize=(13, 4), sharey=True)
...
axs[0].set_title("Sum (M2 + N + P)")
axs[0].axvline(0, color="k", linestyle="-", lw=0.5, zorder=0)

# Transient panel (middle)
sns.lineplot(
    data=transient_first_ano,
    x="time",
    y="N",
...
axs[1].set_title("Transient (M2 + N + P)")
axs[1].axvline(0, color="k", linestyle="-", lw=0.5, zorder=0)
...
axs[2].set_title("Steady (M2 + N + P)")
axs[2].axvline(0, color="k", linestyle="-", lw=0.5, zorder=0)
...
# custom legend on the rightmost axis
decade_handles = [
    Line2D([0], [0], color="black", lw=2, label="1850"),
    Line2D([0], [0], color="red", lw=2, label="2090"),
]
...
leg1 = axs[2].legend(
    handles=decade_handles, title="decade", loc="upper left", frameon=False
)
leg2 = axs[2].legend(
    handles=phase_handles,
    title="phase",
...
plt.tight_layout()
# optional: save
# fig_lastrow.savefig("/work/mh0033/m300883/High_frequecy_flow/docs/plots/0eddy_flux/last_row_lineplots.pdf", dpi=300, bbox_inches="tight", transparent=True, metadata={"Creator": __file__})

# %%

# --- Added: save DataFrames required for the line plots ---
from pathlib import Path

outdir = Path("/work/mh0033/m300883/High_frequecy_flow/data/defense/eddy_momentum")
outdir.mkdir(parents=True, exist_ok=True)

# Full dataframes used to build the line plots
to_save_full = {
    "transient_dfs": transient_dfs,
    "steady_dfs": steady_dfs,
    "sum_dfs": sum_dfs,
    "transient_dfs_clima": transient_dfs_clima,
    "steady_dfs_clima": steady_dfs_clima,
    "sum_dfs_clima": sum_dfs_clima,
}
for name, df in to_save_full.items():
    df.to_csv(outdir / f"{name}.csv", index=False)

# plev-specific anomaly subsets used in the last-row/1x3 line plots
plev_specific = {
    "sum_first_ano": sum_first_ano,
    "sum_last_ano": sum_last_ano,
    "transient_first_ano": transient_first_ano,
    "transient_last_ano": transient_last_ano,
    "steady_first_ano": steady_first_ano,
    "steady_last_ano": steady_last_ano,
}
for name, df in plev_specific.items():
    # include plev in filename to make clear which level was used
    df.to_csv(outdir / f"{name}_plev{int(plev_plot)}.csv", index=False)
