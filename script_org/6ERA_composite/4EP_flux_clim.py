...
def calculate_EP_flux(eddy= 'transient', phase = 'clima', equiv_theta=True, time_window='all', method='no_stat'):
    """
    Calculate EP flux for a given phase
    """
    logging.info(f"Reading data for {phase} phase")
    
    # Read data
    if eddy == 'transient':
        upvp = xr.open_dataset(f"")
