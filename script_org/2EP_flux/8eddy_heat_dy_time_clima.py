...
def cal_E_div(decade, eddy='transient', **kwargs):
    ...
    if eddy == 'transient':
        M2 = read_climatology(var = 'upvp', decade = decade, model_dir='MPI_GE_CMIP6_allplev')
        upvp = read_climatology(var = 'vpetp', decade = decade, model_dir='MPI_GE_CMIP6_allplev')
    else:
        M2 = read_climatology(var = 'upvp', decade = decade, model_dir='MPI_GE_CMIP6_allplev')
        upvp = read_climatology(var = 'vpetp', decade = decade, model_dir='MPI_GE_CMIP6_allplev')
    ...
    _, dupvpdy = E_div(M2, upvp, **kwargs)
    ...
    dupvpdy.to_netcdf(os.path.join(save_dir, f"{eddy}_eddy_heat_dy_clima_{decade}.nc"))
