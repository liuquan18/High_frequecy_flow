...
to_path=/work/mh0033/m300883/High_frequecy_flow/data/MPI_GE_CMIP6_allplev/${var}_NA_daily_ano/r${member}i1p1f1/
tmp_path=/scratch/m/m300883/$var/r${member}i1p1f1/
tmp_ano_path=/scratch/m/m300883/${var}_NA_daily_ano/r${member}i1p1f1/
...
if [ -d "$tmp_ano_path" ]; then
    rm -rf "$tmp_ano_path"
fi
mkdir -p "$tmp_ano_path"

export to_path tmp_path tmp_ano_path
...
split() {
    ...
    cdo -splityear -del29feb $infile $tmp_path$name_prefix
...
merge() {
    ...
    if [ -n "$files" ]; then
        ...
        tmpfile="${tmp_ano_path}${var}_day_MPI-ESM1-2-LR_r${member}i1p1f1_gn_${start_year}0501-${end_year}0930.nc"
        cdo -O -sellevel,50000 -sellonlatbox,-90,40,20,80 -selmonth,5/9 -mergetime -apply,-lowpass,182 [ $files ] $tmpfile
        # daily anomaly
        cdo -O -ydaysub $tmpfile -ydaymean $tmpfile $outfile
