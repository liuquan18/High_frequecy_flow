...
# Plot difference lines
axes[0, 1].plot(Tdiv_phi_diff_first, lat, "k-", label="1850", linewidth=2)
axes[0, 1].plot(Tdiv_phi_diff_last, lat, "k--", label="2090", linewidth=2)
axes[0, 1].axvline(0, color="black", linestyle=":", alpha=0.5)
axes[0, 1].set_xlabel(
    r"$-\frac{\partial}{\partial y} (\overline{u'v'})$ / m $s^{-1}$ day $^{-1}$",
    fontsize=12,
)
axes[0, 1].set_title("Transient Eddy Momentum Forcing", fontsize=14, )
...
axes[1, 0].plot(baroc_diff_first, lat, "k-", label="1850", linewidth=2)
axes[1, 0].plot(baroc_diff_last, lat, "k--", label="2090", linewidth=2)
axes[1, 0].axvline(0, color="black", linestyle=":", alpha=0.5)
axes[1, 0].set_xlabel("Eady growth rate / $day^{-1}$", fontsize=12)
axes[1, 0].set_ylabel("Latitude [°]", fontsize=12)
axes[1, 0].set_title("Lower-level baroclinicity", fontsize=14, )
...
# Plot difference lines
axes[1, 1].plot(steady_diff_first, lat, "k-", label="1850", linewidth=2)
axes[1, 1].plot(steady_diff_last, lat, "k--", label="2090", linewidth=2)
axes[1, 1].axvline(0, color="black", linestyle=":", alpha=0.5)
axes[1, 1].set_xlabel(r"$-\frac{\partial}{\partial p} \left( f_0 \frac{\overline{v'\theta'}}{\overline{\theta}_p} \right)$ / m $s^{-1}$ day $^{-1}$", fontsize=12)
axes[1, 1].set_title(
    "Quasi-stationary Eddy Thermal Feedback", fontsize=14, 
)
...
ax_bar.axvline(0, color="red", linestyle=":", alpha=0.5)
ax_bar.set_xlabel(r"$\Delta$ $-\frac{\partial}{\partial p} \left( f_0 \frac{\overline{v'\theta'}}{\overline{\theta}_p} \right)$ / m $s^{-1}$ day $^{-1}$", fontsize=12, color="red")
ax_bar.tick_params(axis="x", labelcolor="red")
ax_bar.legend(loc="upper right")
