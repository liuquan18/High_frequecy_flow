...
covariances_df = covariances.to_dataframe('covariance').reset_index()

# %%
fig, axes = plt.subplots(2, 3, figsize=(15, 10), sharey=False, sharex=False)

# Plot PDF for transient
sns.histplot(transient_first_df['phi_index'], bins = np.arange(-250, 251, 10), ax=axes[0,1], color='black', label='1850', stat='density', kde=False, alpha=0.5)
sns.histplot(transient_last_df['phi_index'],  bins = np.arange(-250, 251, 10),ax=axes[0,1], color='red', label='2090', stat='density', kde=False, alpha=0.5)
axes[0,1].set_title('Transient')
axes[0,1].legend()

# Plot PDF for steady
sns.histplot(steady_first_df['phi_index'], bins = np.arange(-250, 251, 10), ax=axes[0,2], color='black', label='1850', stat='density', kde=False, alpha=0.5)
sns.histplot(steady_last_df['phi_index'], bins = np.arange(-250, 251, 10), ax=axes[0,2], color='red', label='2090', stat='density', kde=False, alpha=0.5)
axes[0,2].set_title('Steady')
axes[0,2].legend()

# Plot covariance
axes[1,0].plot(covariances_df['decade'], covariances_df['covariance'], marker='o', color='black')
axes[1,0].set_title('Covariance')
