...
#%%
def calc_land_ocean_contrast(theta_dec, lat_slice=slice(50, 70), lon_land=slice(60, 120), lon_ocean=slice(300, 360), lon_win=7, lat_win=3):
    ...
    contrast = (
        theta_dec_ano.sel(lon=lon_land).mean(dim='lon')
        - theta_dec_ano.sel(lon=lon_ocean).mean(dim='lon')
    )
    return contrast
...
#%%
# to pandas
theta_dec_contrast_df = theta_dec_contrast['theta'].to_dataframe('contrast').reset_index()
...
steady_heat_df['steady_heat'] = steady_heat_df['steady_heat'] / steady_heat_df.loc[steady_heat_df['decade'] == 1850, 'steady_heat'].values[0]
...
sns.scatterplot(
    ...
    hue='decade',
    palette='Greys',
    s=150,  # Use 's' for marker size (not 'size')
)
# Set titles and labels for 2x2 axes
axes[0, 0].set_title("")
axes[0, 0].set_ylabel(r"$\overline{\theta^*}$ at 850 hPa (K)")
axes[0, 1].set_title("")
axes[0, 1].set_ylabel(r"$\overline{\theta_e^*}$ at 850 hPa (K)")
axes[1, 0].set_title("")
axes[1, 0].set_ylabel(r"$\overline{zg^*}$ at 250 hPa (m)")
axes[1, 1].set_title("")
axes[1, 1].set_ylabel(r"std of $\partial^2 \overline{v^*\theta^*}/\partial y^2$")
axes[1, 1].legend(loc = 'lower right')
# Remove the top and right spines, set x-labels, legends, and panel labels
for i in range(2):
    for j in range(2):
        ax = axes[i, j]
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ax.set_xlabel("Longitude" if j < 2 and i < 2 and (i, j) != (1, 1) else "")
        if (i, j) != (1, 1):  # Only add legend for line plots
            ax.legend(loc='lower right', fontsize=10)
        # Panel label
        ax.text(0.02, 0.98, f"{chr(97 + i*2 + j)}", transform=ax.transAxes, fontsize=12, va='top', fontweight='bold')
        # Label x-tick every 90 degrees for line plots
        if (i, j) != (1, 1):
            ax.set_xticks(np.arange(-180, 181, 90))

# vertical text "North Atlantic" at x = -50 on the first subplot
axes[0, 0].text(0.43, 0.7, "North Atlantic", transform=axes[0, 0].transAxes, fontsize=10, va='center', ha='right', rotation=90)
axes[0, 0].text(0.73, 0.4, "Eurasia", transform=axes[0, 0].transAxes, fontsize=10, va='center', ha='right', rotation=90)

axes[1, 1].set_xlabel(r"land-ocean contrast [K]")
